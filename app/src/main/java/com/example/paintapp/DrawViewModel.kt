...
class DrawViewModel(private val repository: DrawRepository) : ViewModel() {
    ...
    // Observe all drawings from the repository
    val drawings: LiveData<List<Drawing>> = DrawRepository.getAllDrawings().asLiveData()

    // Local reference to the currently selected drawing
...
//    init {
//        _drawings.value = repository.getAllDrawings()
//    }
    ...
    private fun addNewDrawingToRepo(newDrawing: Drawing) {
        viewModelScope.launch {
            // Request the repository to add the new drawing to the database
            repository.addDrawing(newDrawing)
        }
    }

    // Inside your DrawViewModel
    //TODO: pass fileName here so user can pick their filename
    fun createNewDrawing(userFileName: String) {
        val newBitmap = Bitmap.createBitmap(800, 800, Bitmap.Config.ARGB_8888) // Create a blank bitmap

        //Unresolved reference saveBitmapToFile can you show the code for this method to put in my repo?
        val newPath = repository.saveBitmapToFile(newBitmap) // No value passed for parameter 'fileName'

        val newDrawing = Drawing(
            id = 0L,  // Room will auto-generate the ID
            actualPathToBitmap = newPath, // Temporary, to be updated later
            userFileName = userFileName,
            bitmap = newBitmap // Assign a non-null bitmap
        )
        //add the new drawing to the repo
        addNewDrawingToRepo(newDrawing)

        //update local the reference to the selected drawing
        selectDrawing(newDrawing)
    }

    // Update the bitmap of the selected drawing, Ffnction to save the current drawing's bitmap
    fun saveCurrentDrawing(newBitmap: Bitmap) {
        _selectedDrawing.value?.let { drawing ->
            ...
            repository.updateDrawingInList(updatedDrawing)
        }
    }
